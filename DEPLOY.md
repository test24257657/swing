...
3. **Manual Deploy → Deploy latest commit.**
   On boot the app creates the `users` table if missing and seeds the admin from
   `ADMIN_EMAIL` / `ADMIN_PASSWORD` (`app/bootstrap.py`). No migration step, no DB
   access during the build — so `DATABASE_URL` **must** be set before this deploy.
...
- **Change the admin password:** update `ADMIN_PASSWORD` in Render → redeploy (the
  startup bootstrap refreshes the existing row).
...
pip install -e '.[dev]'
OUT_DIR=../../out uvicorn app.main:app --reload --port 8000   # creates users + seeds admin on startup
